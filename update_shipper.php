...
if (isset($_GET['id'])) {
    $id          = $_GET['id'];
    $sql         = 'select shipper.id,shipper.description , shipper.name, shipper.image, shipper.nhacungcap_id ,nhacungcap.name as nhacungcap_name from shipper inner join nhacungcap on shipper.nhacungcap_id = nhacungcap.id where shipper.id = ' . $id;
    $shipper = executeResult($sql);
    if ($shipper != null && count($shipper) > 0) {
        $prd = $shipper[0];
        $name = $prd['name'];
        $nhacungcap_id = $prd['nhacungcap_id'];
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['nhacungcap'])) {
        $nhacungcap_id = $_POST['nhacungcap'];
    }

    if (isset($_POST['description'])) {
        $description = $_POST['description'];
    }
    $sql = "update shipper set name = '$name', image = '$image', nhacungcap_id = '$nhacungcap_id', description = '$description' where id = " . $id;
    execute($sql);
...
<body class="hold-transition sidebar-mini layout-fixed">
        ...
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-10 text-center">
              <h1 class="m-0">Sửa thông tin shipper</h1>
            </div><!-- /.col -->
            <div class="col-sm-2">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="col-6 input_group">
            ...
            <textarea name="description" id="input" class="form-control" rows="3" required="required"><?php echo $description; ?></textarea>
        ...
        <div class="col-6 input_group">
            ...
            <select name="nhacungcap" id="input" class="form-control" required="required">
                <option value="">Lựa chọn nhà cung cấp</option>
                <?php foreach ($nhacungcapp as $nhacungcap) {
                    if ($nhacungcap_id == $nhacungcap['id']) {
                ?>
                        <option value="<?php echo $nhacungcap['id']; ?>" selected><?php echo $nhacungcap['name'] ?> </option>
                    <?php } else { ?>
                        <option value="<?php echo $nhacungcap['id']; ?>"><?php echo $nhacungcap['name'] ?> </option>
                <?php }
                } ?>
