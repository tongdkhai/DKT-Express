        ...
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-10 text-center">
              <h1 class="m-0">Danh sách shipper</h1>
            </div><!-- /.col -->
            <div class="col-sm-2">
                ...
                <div class="card-body">
                  ...
                  <table class="table">
                    ...
                    <tbody>
                      ...
                      if (isset($_GET['search']) && $_GET['search'] != '' && isset($_GET['nhacungcap']) && $_GET['nhacungcap'] != '') {
                        $sql = 'select shipper.id, shipper.name, shipper.image, shipper.description, nhacungcap.name as nhacungcap_name from shipper inner join nhacungcap on shipper.nhacungcap_id = nhacungcap.id where shipper.name like "%' . $_GET['search'] . '%" and shipper.nhacungcap_id = ' . $_GET['nhacungcap'];
                      }
                      if (!isset($_GET['search']) && !isset($_GET['nhacungcap'])) {
                        $sql = 'select shipper.id, shipper.name, shipper.image, shipper.description, nhacungcap.name as nhacungcap_name from shipper inner join nhacungcap on shipper.nhacungcap_id = nhacungcap.id';
                      }
                      $shippers = executeResult($sql);

                      $index = 1;
                      foreach ($shippers as $shipper) {
                        echo '<tr>
                              <td>' . ($index++) . '</td>
                              <td><img width = 100px height= 100px src=' . $shipper['image'] . ' ></td>
                              <td>' . $shipper['name'] . '</td>
                              <td>' . $shipper['description'] . '</td>
                              <td>' . $shipper['nhacungcap_name'] . '</td>
                              <td><button class="btn btn-success" onclick=\'window.open("update_shipper.php?id=' . $shipper['id'] . '","_self")\'>Sửa</button>
                                  <button class="btn btn-danger" onclick="deleteShipper(' . $shipper['id'] . ')">Xóa</button></td>
                            </tr>';
                      }
                      ?>
                    </tbody>
